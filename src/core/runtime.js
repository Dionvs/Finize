...
function createGoalRecord(owner){
  return {id:uid(),naam:'Nieuw doel',doelbedrag:0,algespaard:0,doeldatum:'',vasteInleg:0,vastBedrag:false,rendement:.0125,rendementPeriode:'jaarlijks',favoriet:false,eigenaar:owner,ratoVerdeling:true,subdoelen:[]};
}
...
function bindDirectGoalOrdering(root){
  if(root.id==='tab-spaardoelen'){
    ...
    root.querySelectorAll('.mobile-goal-section').forEach(section=>{
      ...
      section.querySelectorAll('.mobile-goal-row').forEach(row=>{
        ...
        if(!card.querySelector(':scope > .goal-direct-order-controls')){
          const label=attrSafe(row.querySelector('strong')?.textContent||'Spaardoel');
...
function transactionMatchesLegacyRefund(tx,refund){
  ...
  const transactionText=bankText(tx?.description||tx?.title||tx?.name||'');
...
function unmatchedMonthlyRefundTotal(owner,month,transactions){
  return round2((state.monthlyTeruggaven?.[month]?.[owner]||[]).reduce((sum,refund)=>{
...
const CloudAdapter = {
  ...
  attachSnapshot(){
    ...
    this.unsubscribe = firestore.onSnapshot(this.docRef, async snap=>{
      ...
      const cloudChanged = this.initialSyncComplete
        && (remoteVersion !== this.cloudVersion || remoteSignature !== this.lastCloudSignature);
  ...
  queueSave(state){
    ...
    this.pendingState = clone(state);
    this.status = this.conflict
      ? 'Synchronisatieconflict'
      : (this.isConnected() ? 'Opslaan…' : 'Offline — lokaal bewaard');
    renderCloudStatus();
...
function renderCloudStatus(){
  ...
  const online = CloudAdapter.isConnected()
    && status !== 'Synchronisatie mislukt'
    && status !== 'Synchronisatieconflict';
  if (save){
...
function openMobileGoalEditor(owner,id){
  ...
  imageInput.addEventListener('change',async event=>{
    ...
    try{
      ...
      goalImageData=safeImageUrl(goalImageData);
      imagePreview.style.backgroundImage=goalImageData?`url('${goalImageData}')`:''; imagePreview.textContent='';
  ...
  modal.querySelector('#goalEditSave').addEventListener('click',async ()=>{
    ...
    try{
      ...
      const selectedOwner=modal.querySelector('#u2GoalOwner')?.value;
      const targetOwner=manageableGoalOwnerKeys().includes(selectedOwner)?selectedOwner:owner;
      if(!commitChange(()=>{updateItemById(`spaardoelen.${targetOwner}`,id,changes);u2SetGoalSavedAmount(goal,savedAmount);},{render:false})) throw new Error('Lokale opslag is mislukt.');
      if (!goalImageData){
...
service cloud.firestore {
  ...
  document.getElementById('btnExport').addEventListener('click', async ()=>{
    try{
      ...
      alert('De back-up kon niet worden gemaakt: ' + (error?.message || String(error)));
    }
  });
  ...
  document.getElementById('btnRestoreBackup').addEventListener('click', async ()=>{
    ...
    if (confirm('Laatste lokale nood-back-up herstellen van ' + label + '? De huidige stand wordt eerst opnieuw als nood-back-up bewaard.')){
      try{
        ...
        alert('De lokale nood-back-up kon niet worden hersteld: ' + (error?.message || String(error)));
      }
    }
  });
  ...
  document.getElementById('btnReloadCloud').addEventListener('click', async ()=>{
    ...
    try{
      ...
      CloudAdapter.status = 'Synchronisatie mislukt';
      renderCloudStatus();
      alert('De cloudstand kon niet opnieuw worden geladen: ' + (error?.message || String(error)));
    }
  });
...
function openTransactionEntryMenu(owner){
  ...
  bindModalBackdrop(modal,close);
  modal.querySelector('[data-entry-manual]').addEventListener('click',()=>{close();owner==='gezamenlijk'?openJointTransactionModal():openPersonalTransactionModal(owner);});
...
function renderActiveTab(){
  ...
  root.querySelectorAll('.mobile-savings-overview .manage-section').forEach(section=>{
