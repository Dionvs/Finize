(() => {
  ...
  function u3RecognitionFromLegacy(rule) {
    return {
      ...
      category: String(rule.category || "Overig"),
    ...
    ["voor", "na"].forEach((scenario) => {
      ...
      U3_ACCOUNTS.forEach((account) => {
        ...
        groups.forEach((group) => group.rows.forEach((row) => {
          ...
          existing.push({
            id: `fixed-${scenario}-${account}-${row.id}`,
            legacyKey,
  ...
  function dashboardIncomeBreakdown(month = getSelectedMonth()) {
    ...
    rows.forEach((tx) => {
      ...
      if (type === "salaris" && (owner === "dion" || owner === "dara" || owner === "gezamenlijk")) {
  ...
  var FINIZE_NAV_ICONS = { dashboard: "dashboard", gezamenlijk: "shared", dion: "person", dara: "person", spaardoelen: "target", data: "cloud" };
  ...
  function goalStatusBadge(progress) {
    ...
    if (progress >= 0.75) return '<span class="status-badge amber">Bijna klaar</span>';
  ...
  var bankImportDraft = null;
  var bankImportOpen = false;
  ...
  async function compressGoalImage(file) {
    ...
    if (file.size > 12e6) throw new Error("Kies een afbeelding kleiner dan 12 MB.");
    let bitmap = null;
    let sourceUrl = "";
    ...
    try {
      ...
      for (let pass = 0; pass < 3; pass++) {
        ...
        for (const quality of [0.82, 0.72, 0.62, 0.52, 0.44]) {
          ...
          if (dataUrlByteSize(result) <= 85e3) return result;
        }
  ...
  function calcGroep(doelen, spaarpotDezeMaand, today) {
    ...
    while (resterendePotCents > 0 && active.length) {
      ...
      if (capped.length) {
        ...
        capped.forEach((item) => {
          const amount = capacities[item.index];
          extraCents[item.index] = amount;
          resterendePotCents -= amount;
        });
        active = active.filter((item) => !cappedIndexes.has(item.index));
        continue;
      }
      const shares = active.map((item) => {
        const raw = resterendePotCents * (item.weight / totalWeight);
      ...
      shares.forEach((item) => {
        extraCents[item.index] = Math.min(item.cents, capacities[item.index]);
      });
  ...
  function calcScenario(state2) {
    ...
    return {
      ...
      hypotheekBedrag,
      variabelTotaal,
      variabelBudgetTotaal,
      variabelVoorVerdelingTotaal,
      spaarpotDezeMaand,
      effDion,
      effDara,
      dion,
  ...
  function defaultState() {
    ...
    return {
      ...
      voor: {
        verdeling: { minimumDion: 0.4 },
        spaarpotDezeMaand: 0,
        gezamenlijk: {
          vasteLasten: [
            { id: uid(), categorie: "Huis", post: "Hypotheek", bedrag: 2e3 },
        ...
        dion: {
          ...
          variabel: ["Eten buiten de deur", "Benzine/vervoer", "Uitjes", "Kleding/hobby", "Overig"].map((post) => ({ id: uid(), categorie: "Variabel", post, bedrag: 0 }))
        },
        dara: {
          vasteLasten: [
      ...
      spaardoelen: {
        gezamenlijk: [
          { id: uid(), naam: "vakantie", doelbedrag: 5e3, algespaard: 100, doeldatum: "2028-12-31", vasteInleg: 0, rendement: 0.0125 },
          { id: uid(), naam: "buffer", doelbedrag: 1e4, algespaard: 0, doeldatum: "2030-12-31", vasteInleg: 0, rendement: 0.0125 },
          { id: uid(), naam: "Beleggen", doelbedrag: 5e4, algespaard: 17941, doeldatum: "2030-12-31", vasteInleg: 0, rendement: 0.08, vastBedrag: true }
        ],
        dion: [
          { id: uid(), naam: "koffie", doelbedrag: 300, algespaard: 6.77, doeldatum: "2026-12-31", vasteInleg: 0, rendement: 0.0125 },
          { id: uid(), naam: "buffer", doelbedrag: 5e3, algespaard: 800, doeldatum: "2028-12-31", vasteInleg: 0, rendement: 0.0125 },
          { id: uid(), naam: "Beleggen", doelbedrag: 5e4, algespaard: 17941, doeldatum: "2030-12-31", vasteInleg: 250, rendement: 0.08, vastBedrag: true }
        ],
        dara: []
      },
      monthlyIncome: {},
  ...
  var GoalImageStore = {
    ...
    ref(goalId) {
      return GOAL_IMAGE_REF_PREFIX + goalId;
    },
    keyFromRef(value) {
  ...
  function backupLabel() {
    return (/* @__PURE__ */ new Date()).toLocaleString("nl-NL");
  }
  ...
  var CloudAdapter = {
    ...
    isConfigured() {
      return firebaseConfigIsComplete(this.config);
    },
    isConnected() {
      return !!(this.db && this.docRef);
    },
    async loadModules() {
  ...
  function bindInputs(root2) {
    ...
    root2.querySelectorAll("[data-path]").forEach((el) => {
      ...
      else el.value = raw === void 0 || raw === null ? "" : raw;
      const commit = () => {
        let v = el.value;
        if (type === "number") {
  ...
  function handleTableClicks(root2) {
    ...
    root2.querySelectorAll("[data-move-row-id][data-source-path]").forEach((select) => {
      select.addEventListener("change", () => {
        ...
        let movement = null;
        if (!commitChange(() => {
          movement = moveItemById(sourcePath, targetPath, id);
        }, { render: false }) || !movement) return;
        renderActiveTab();
  ...
  function openMobileGoalEditor(owner, id) {
    ...
    imageField.querySelector("#goalEditImageRemove").addEventListener("click", () => {
      goalImageData = "";
      imagePreview.style.backgroundImage = "";
      imagePreview.textContent = "Geen afbeelding";
      imageInput.value = "";
      imageStatus.textContent = "Afbeelding verwijderd. Sla het doel op om dit te bevestigen.";
      imageField.querySelector("#goalEditImageRemove").disabled = true;
    });
    ...
    modal.querySelector("#goalEditSave").addEventListener("click", async () => {
      ...
      try {
        ...
        if (!goalImageData) {
          try {
            await GoalImageStore.remove(id);
          } catch (error) {
            console.warn("Verwijderde spaardoelfoto kon niet uit IndexedDB worden opgeruimd.", error);
          }
        }
    ...
    modal.querySelector("#goalEditDelete").addEventListener("click", () => {
      close();
      removeWithUndo(`spaardoelen.${owner}`, id, "Spaardoel verwijderd");
    });
  }
  function openMobileGoalManager(owner) {
    ...
    modal.querySelectorAll("[data-open-manager-goal]").forEach((btn) => {
      const open = () => {
        modal.classList.remove("open", "goal-manager-editor-open");
        openMobileGoalEditor(owner, btn.dataset.openManagerGoal);
      };
    ...
    modal.querySelectorAll("[data-move-manager-goal]").forEach((btn) => btn.addEventListener("click", (event) => {
      event.stopPropagation();
      const [id, deltaText] = btn.dataset.moveManagerGoal.split(":");
      const index = goals.findIndex((goal) => goal.id === id);
      const target = index + Number(deltaText);
      if (index < 0 || target < 0 || target >= goals.length) return;
      commitChange(() => moveItemById(`spaardoelen.${owner}`, `spaardoelen.${owner}`, id, target), { render: false });
      openMobileGoalManager(owner);
    }));
    modal.querySelectorAll("[data-remove-manager-goal]").forEach((btn) => btn.addEventListener("click", (event) => {
      event.stopPropagation();
      modal.classList.remove("open", "goal-manager-editor-open");
...
service cloud.firestore {
  ...
  function openBudgetTransactionsModal(category, owner = "gezamenlijk") {
    ...
    modal.querySelectorAll("[data-open-budget-transaction-id]").forEach((button) => button.addEventListener("click", () => {
      ...
      close();
      owner === "gezamenlijk" ? openJointTransactionModal(id) : openPersonalTransactionModal(owner, id);
    }));
  }
  ...
  function placeDesktopPageHeading(root2 = document.getElementById("tab-" + activeTab)) {
    ...
    if (window.innerWidth >= 1024) {
      parkedHeadings.forEach((item) => {
        if (item !== heading) item.remove();
      });
  ...
  function u2ExpectedDate(goal, monthly) {
    ...
    for (let month = 1; month <= 1200; month++) {
      ...
      if (balance + 5e-3 >= target) return new Date(date);
    }
    return null;
  }
  function u2DateLabel(date) {
    return date instanceof Date && !isNaN(date) ? date.toLocaleDateString("nl-NL", { day: "numeric", month: "long", year: "numeric" }) : "Niet berekenbaar";
  }
  ...
  function u2DistributeCents(totalCents, candidates, capacities) {
    ...
    while (remaining > 0 && active.length) {
      ...
      if (capped.length) {
        const cappedIds = /* @__PURE__ */ new Set();
        capped.forEach((item) => {
  ...
  calcGroep = function(doelen, spaarpotDezeMaand, today) {
    ...
    if (!onvoldoende) {
      const candidates = berekend.map((b, index) => ({ index, weight: Math.max(0, Number(b.benodigdPerMaand) || 0) })).filter((item) => berekend[item.index].doel.ratoVerdeling && item.weight > 0 && capacities[item.index] - vasteCents[item.index] > 0);
  ...
  function u2OpenProcessModal(owner) {
    ...
    const render2 = () => {
      ...
      modal.querySelector("[data-u2-confirm]").addEventListener("click", () => {
        ...
        if (total2 > pot + 5e-3) {
          modal.querySelector("[data-u2-error]").textContent = "De verdeling is hoger dan de beschikbare spaarpot.";
          return;
        }
        ...
        modal.classList.remove("open", "u2-process-open");
        modal.innerHTML = "";
        showQuickToast("Spaarpot verwerkt");
        renderActiveTab();
      });
      refresh();
    };
  ...
  openMobileGoalEditor = function(owner, id) {
    ...
    const renderChildren = () => {
      ...
      section.querySelectorAll("[data-u2-child-remove]").forEach((btn) => btn.addEventListener("click", () => {
        sync();
        drafts.splice(Number(btn.dataset.u2ChildRemove), 1);
        renderChildren();
      }));
      const move = (from, to) => {
        sync();
        if (to < 0 || to >= drafts.length) return;
        const [child] = drafts.splice(from, 1);
        drafts.splice(to, 0, child);
        renderChildren();
      };
      ...
      section.querySelectorAll("[data-u2-child]").forEach((row) => {
        row.addEventListener("dragstart", () => {
          sync();
          dragIndex = Number(row.dataset.u2Child);
        ...
        row.addEventListener("drop", (event) => {
          event.preventDefault();
          const target = Number(row.dataset.u2Child);
    ...
    modal.querySelector("#goalEditSave").addEventListener("click", () => {
      ...
      commitChange(() => {
        ...
        if (newOwner !== owner) {
          ...
          state.spaardoelen[newOwner].push(moved);
        }
      }, { render: false });
    }, true);
  ...
  function u3DeterministicTransferId(closureId, type, source, target) {
    return `transfer-${closureId}-${type}-${source}-${target}`.replace(/[^a-zA-Z0-9_-]/g, "-");
  }
  ...
  function u3DeactivateClosingEffects(closingId) {
    if (!closingId) return;
    (state.reserveLedger || []).forEach((row) => {
      if ((row.sourceClosingId || row.closureId) === closingId) row.status = "vervallen";
    });
    (state.internalTransfers || []).forEach((row) => {
      if ((row.sourceClosingId || row.closureId) === closingId) row.status = "vervallen";
    });
    (state.monthCorrections || []).forEach((row) => {
      if ((row.sourceClosingId || row.closureId) === closingId) row.status = "vervallen";
    });
    (state.savingsGoalLedger || []).forEach((row) => {
      if (row.sourceClosingId !== closingId) return;
      row.active = false;
      row.status = "teruggedraaid";
  ...
  function u3CloseMonth(month = getSelectedMonth(), actualBalances = {}, correctionAccounts = [], options = {}) {
    ...
    if (previous) {
      ...
      previous.status = "vervangen";
    }
    drafts.filter((row) => row.calculatedAmount > 4e-3).forEach((row) => {
      if (!state.internalTransfers.some((item) => item.id === row.id)) state.internalTransfers.push(row);
    });
    U3_ACCOUNTS.forEach((owner) => {
      const id = `reserve-${closureId}-${owner}`;
      if (!state.reserveLedger.some((row) => row.id === id)) state.reserveLedger.push({ id, month, owner, amount: summary.reserve[owner], reason: "Netto budgetverschil", closureId, sourceClosingId: closureId, status: "actief", createdAt: (/* @__PURE__ */ new Date()).toISOString() });
    });
  ...
  function u3ReopenMonth(month = getSelectedMonth()) {
    ...
    const closure = record.closureHistory.find((item) => (item.closingId || item.id) === record.activeClosureId);
    if (closure) {
      closure.status = "teruggedraaid";
      closure.reopenedAt = (/* @__PURE__ */ new Date()).toISOString();
      u3DeactivateClosingEffects(closure.closingId || closure.id);
    }
    record.status = "open";
    record.reopenedAt = (/* @__PURE__ */ new Date()).toISOString();
    record.activeClosureId = "";
    return true;
  }
  ...
  function u3AccountLabel(value) {
    return ownerLabel(value || "gezamenlijk");
  }
  function u3AdminModal(html, extraClass = "") {
    ...
    const close = () => {
      modal.classList.remove("open", "u3-admin-open");
      modal.innerHTML = "";
      renderActiveTab();
    };
  ...
  function u3OpenReview() {
    ...
    modal.querySelectorAll("[data-u3-ignore-review]").forEach((button) => button.addEventListener("click", () => {
      commitChange(() => {
        ...
        if (row) row.reviewStatus = "genegeerd";
      }, { render: false });
      u3OpenReview();
    }));
    modal.querySelectorAll("[data-u3-confirm-review]").forEach((button) => button.addEventListener("click", () => {
      try {
        ...
        commitChange(() => {
          ...
          source.reviewStatus = "bevestigd";
          source.confirmedTransactionId = tx.id;
  ...
  function u3OpenClose() {
    ...
    modal.querySelectorAll("[data-u3-save-opening]").forEach((button) => button.addEventListener("click", () => {
      const account = button.dataset.u3SaveOpening;
      const value = bankAmount(modal.querySelector(`[data-u3-opening="${account}"]`).value);
      if (!Number.isFinite(value)) {
        alert("Vul een geldig openingssaldo in.");
        return;
      }
      commitChange(() => {
    ...
    const closeMonth = (force = false) => {
      try {
        ...
        const corrections = [];
        U3_ACCOUNTS.forEach((account) => {
          const value = bankAmount(modal.querySelector(`[data-u3-actual="${account}"]`).value);
          if (Number.isFinite(value)) actual[account] = value;
          if (modal.querySelector(`[data-u3-correct="${account}"]`).checked) corrections.push(account);
        });
        commitChange(() => u3CloseMonth(month, actual, corrections, { force }), { render: false });
        u3OpenClose();
      } catch (error) {
        alert(error.message);
      }
  ...
  (function(root2, factory) {
    ...
    function detectDelimiter(text) {
      ...
      for (const char of first) {
        if (char === '"') quoted = !quoted;
          ...
          const hit = counts.find((item) => item[0] === char);
          if (hit) hit[1]++;
        }
      }
    ...
    function organizationName(description) {
      return normalizeText(description).replace(/\b(pasvolgnr|betaalautomaat|incasso|ideal|sepa|europese|betaling|kenmerk|omschrijving)\b.*$/, "").replace(/\b\d{3,}\b/g, "").trim();
    }
    ...
    function createImportDraft({ text, fileName = "import.csv", profiles = [], rules = [], transactions = [], id = uid2("import") }) {
      ...
      return {
        ...
        rows,
        summary: { newCount: active.length, duplicateCount: rows.filter((row) => row.duplicate).length, totalIncome: round22(income), totalExpenses: round22(expenses), sureCount: active.filter((row) => row.certainty === "zeker").length, reviewCount: active.filter((row) => row.certainty === "nakijken").length }
      };
    }
    ...
    async function reconcileActiveImportReference(root2, { localRead = (id) => ImportStore.getImport(id) } = {}) {
      ...
      let local = null;
      try {
        local = await localRead(activeId);
      } catch (error) {
        return { action: "local-error", activeImportId: activeId, error };
      }
      if (local) {
    ...
    function planImportEffects(draft, state2) {
      ...
      for (const row of draft.rows.filter((item) => item.bankOriginal.valid && !item.duplicate)) {
        ...
        financialRows(row).forEach((part) => {
          ...
          const saving = savingsForTransaction(tx, state2);
          if (saving) savingsEntries.push(saving);
          const advance = advanceForTransaction(tx);
          if (advance) {
    ...
    function undoImportEffects(state2, draft) {
      ...
      state2.advanceRepayments = state2.advanceRepayments.filter((item) => !repaymentIds.has(item.id));
      state2.savingsGoalLedger = state2.savingsGoalLedger || [];
      const affectedSavingGoals = state2.savingsGoalLedger.filter((item) => savingIds.has(item.id)).map((entry) => entry.goalId);
    ...
    async function undoImport(root2, draft) {
      ...
      if (!ok) {
        ...
        journal.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
        await ImportStore.putJournal(journal);
        throw new Error("Ongedaan maken is afgebroken; er zijn geen halve wijzigingen bewaard.");
      }
      draft.status = "teruggedraaid";
      draft.undoneAt = (/* @__PURE__ */ new Date()).toISOString();
      await ImportStore.putImport(draft);
      await queueImportSync(draft);
    ...
    function matchCandidates(draft, source) {
      ...
      return draft.rows.filter((row) => {
        ...
        if (sourceIdentity.account && identity.account && sourceIdentity.account === identity.account) {
          score += 6;
          reasons.push("zelfde tegenrekening");
        }
        if (sourceIdentity.organization && identity.organization && sourceIdentity.organization === identity.organization) {
          score += 4;
          reasons.push("zelfde organisatie");
        }
        if (sourceIdentity.description && identity.description && sourceIdentity.description === identity.description) {
          score += 3;
...
${(error == null ? void 0 : error.message) || error}`);
    ...
    function renderRules(root2) {
      ...
      modal.querySelector("[data-u4-close]").addEventListener("click", closeDraft);
      modal.addEventListener("change", (event) => {
        const card = event.target.closest("[data-u4-rule]");
        if (!card) return;
        const rule = rules.find((item) => item.id === card.dataset.u4Rule);
        if (!rule) return;
  ...
  (function() {
    ...
    function calculatedGoals() {
      return goalGroups().flatMap(
        (group) => calcGroep(group.goals, group.pot, TODAY).map((item) => ({ ...item, owner: group.owner, ownerLabel: group.label }))
      );
    }
    ...
    function goalDetail(item) {
      ...
      <div class="u5-goal-detail-grid">
        ...
        <div><span>Verwacht rendement</span><strong>${Math.round((Number(goal.rendement) || 0) * 1e4) / 100}% ${goal.rendementPeriode === "maandelijks" ? "per maand" : "per jaar"}</strong></div>
  ...
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("./service-worker.js").catch((error) => console.error("Service worker registratie mislukt:", error));
    });
  }
})();
