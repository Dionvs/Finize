...
De cachemarker is `finize-v73-cloud-leidend`. Alleen caches met de prefix `finize-` worden opgeruimd. Alleen navigatieverzoeken mogen offline op `index.html` terugvallen; ontbrekende scripts, CSS en afbeeldingen krijgen nooit HTML als vervanging. Optionele pictogrammen kunnen een installatie niet blokkeren.

Cloudwrites gebruiken een Firestore-transactie met een serverbrede `syncVersion` en unieke `commitId`. De eerste geldige cloudsnapshot is leidend; een apparaat-lokale `revision` bepaalt nooit meer welke apparaatstand wint. Als de cloud sinds de laatste bevestiging is veranderd, wordt de lokale stand als nood-back-up bewaard en daarna door de actuele cloudstand vervangen. Alleen expliciet herstel van een JSON- of noodback-up wordt als nieuwe wijziging opgeslagen.
